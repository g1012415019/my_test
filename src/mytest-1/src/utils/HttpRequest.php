...
class HttpRequest{
     ...
     * 设置请求参数
     * @author gongzhe
     * @createTime 2018-07-09 16:28:38
     * @qqNumber 1012415019
     */
    public function setConfig($config=[]){

        $httpEncryptRequest=new HttpEncryptRequest();

        //设置加密参数
        $httpEncryptRequest->setConfig([
            'data'      =>$this->formData,
            'method'    =>empty($config['method'])?'get':$config['method'],
            'url_key'   =>empty($config['type'])?'':$config['type'],
        ]);

        //默认需要生成签名
        $config['is_sign']=empty($config['is_sign'])?true:$config['is_sign'];

        //获得签名后的url
        $this->url=$config['is_sign']!=true?$httpEncryptRequest->getUrl():$httpEncryptRequest->getSignUrl();
     ...
     * @qqNumber 1012415019
     */
    public function setNotSignConfig($config=[]){
     ...
     * 设置请求表单数据
     * @author gongzhe
     * @createTime 2018-07-09 16:29:23
     * @qqNumber 1012415019
     */
    public function setFormData($formData=[]){

        //追加额外数据
        $formData= array_merge($this->setExtendData(),$formData);
    ...
    public function setFileData($fileData=[]){
        ...
        $fileData=array_merge($this->setExtendData(),$fileData);
     ...
     * 设置扩展参数
     * @author gongzhe
     * @createTime 2018-07-11 17:09:06
     * @qqNumber 1012415019
     */
    private function setExtendData(){

        $data=[];
        $data['access_source']    = $this->getAgentType();
        $data['access_browser']   = $this->getBrowser().$this->getBrowserVer();
        $data['access_ipaddress'] = $this->real_ip();
        $data['access_is_mobile'] = $this->is_mobile()?1:0;
        return $data;
